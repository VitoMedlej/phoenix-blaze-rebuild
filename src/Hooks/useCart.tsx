...
const useCart = () => {
    ...
    const incrementQty = (_id:string, newValue?: number, productselectedSize?: string, productselectedColor?: any, newPrice?: number) => {
        ...
        if (foundIndex !== -1 && selectedItem) {
            ...
            if (productselectedSize) {
                ...
                if (newPrice) {
                    selectedItem.price = newPrice;  // Update the price when the size changes
                }
            }
    ...
    const addToCart = (selectedQuantity = 1, _id: string, product: { title: string, category?: string, img: string, _id: string, price: number, productselectedSize?: string, productselectedColor?: any }, open = true, replaceOld = false) => {
        ...
        const increased = incrementQty(_id, selectedQuantity, product.productselectedSize, product.productselectedColor, product.price);
        if (increased) {
